...
When you create a new project, Web PaaS creates a default [low-disk email notification](#low-disk-warning) for all [Project Admins](../administration-users#user-roles).
...
You can view an email notification by running `webpaas integration:get`.

```bash
webpaas integration:get
+--------------+---------------+
| Property     | Value         |
+--------------+---------------+
| id           | abcdefghijklm |
...
```bash
webpaas integration:update abcdefghijklm --recipients you@example.com
```
...
```bash
webpaas integration:add --type health.email --recipients them@example.com --recipients others@example.com
```

The default `from-address` points to the "Web PaaS Bot". You must specify one or more `recipients`, each as its own switch.
