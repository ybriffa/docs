...
slug: development-transfer-dedicated
section: Development
order: 5
---
...
Web PaaS automatically creates a backup of the staging and production instances on a Dedicated cluster every six hours.  However, those are only useful for a full restore of the environment and can only be done by the Web PaaS team.  At times, you'll want to make a manual backup yourself.
...
To transfer data into either the staging or production environments, you can either download it from your Web PaaS Development environment to your local system first or transfer it directly between environments using SSH based tools (e.g. SCP, Rsync).

First, set up [SSH forwarding](../development-ssh#forwarding-keys-by-default) by default for Web PaaS domains.

Then run `webpaas ssh` with the `master` branch checked out to connect to the master dev environment.  Files are the easier data to transfer, and can be done with `rsync`.
...
Now, in a separate terminal login to the development instance using `webpaas ssh`.  Run the same `echo` command as above to get the credentials for the database on the development instance.  (The JSON will be slightly different but again we're only interested in the user, password, host, and "path"/database name).
